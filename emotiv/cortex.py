...
if sys.version_info < (2, 7) or (3, 0) <= sys.version_info < (3, 4):
    print(
        f"[ERROR] Python 2.7+ or 3.4+ is required. You are using Python {sys.version_info.major}.{sys.version_info.minor}.",
        file=sys.stderr,
    )
    sys.exit(1)
...
try:
    ...
    print(
        f"[ERROR] Required library 'websocket-client' is not installed. Please run: {sys.executable} -m pip install websocket-client",
        file=sys.stderr,
    )
    sys.exit(1)
...
try:
    ...
    print(
        f"[ERROR] Required library 'python-dispatch' is not installed. Please run: {sys.executable} -m pip install python-dispatch",
        file=sys.stderr,
    )
    sys.exit(1)
...
class Cortex(Dispatcher):
    ...
    _events_ = [
        ...
        "warn_record_post_processing_done",
        "inject_marker_done",
        "update_marker_done",
        "export_record_done",
        "new_data_labels",
    ...
    def open(self):
        ...
        self.ws = websocket.WebSocketApp(
            url, on_message=self.on_message, on_open=self.on_open, on_error=self.on_error, on_close=self.on_close
        )
    ...
    def handle_result(self, recv_dic):
        ...
        elif req_id == TRAINING_ID:
            # handle response to training request (id == 9)
            # result_dic example: {'action': 'neutral', 'message': 'Set up training successfully', 'status': 'start'}
            print("training response:", result_dic)
            self.emit("training_done", data=result_dic)
        else:
    ...
    def on_message(self, *args):
        ...
        if "sid" in recv_dic:
            ...
            print("Unknown message received from websocket:", recv_dic)
            raise KeyError

    def query_headset(self):
        print("query headset --------------------------------")
        query_headset_request = {"jsonrpc": "2.0", "id": QUERY_HEADSET_ID, "method": "queryHeadsets", "params": {}}
        if self.debug:
    ...
    def create_session(self):
        ...
        create_session_request = {
            ...
            "params": {"cortexToken": self.auth, "headset": self.headset_id, "status": "active"},
        }
    ...
    def close_session(self):
        ...
        close_session_request = {
            ...
            "params": {"cortexToken": self.auth, "session": self.session_id, "status": "close"},
        }
    ...
    def get_cortex_info(self):
        ...
        get_cortex_info_request = {"jsonrpc": "2.0", "method": "getCortexInfo", "id": GET_CORTEX_INFO_ID}
    ...
    def sub_request(self, stream):
        ...
        sub_request_json = {
            "jsonrpc": "2.0",
            "method": "subscribe",
            "params": {"cortexToken": self.auth, "session": self.session_id, "streams": stream},
            "id": SUB_REQUEST_ID,
        }
        if self.debug:
    ...
    def setup_profile(self, profile_name, status):
        ...
        setup_profile_json = {
            "jsonrpc": "2.0",
            "method": "setupProfile",
            "params": {"cortexToken": self.auth, "headset": self.headset_id, "profile": profile_name, "status": status},
            "id": SETUP_PROFILE_ID,
        }
    ...
    def update_marker_request(self, marker_id, time, **kwargs):
        print("update marker --------------------------------")
        params_val = {"cortexToken": self.auth, "session": self.session_id, "markerId": marker_id, "time": time}
    ...
    def get_mental_command_action_sensitivity(self, profile_name):
        ...
        sensitivity_request = {
            ...
            "params": {"cortexToken": self.auth, "profile": profile_name, "status": "get"},
        }
        if self.debug:
    ...
    def set_mental_command_active_action(self, actions):
        ...
        command_active_request = {
            ...
            "jsonrpc": "2.0",
            "method": "mentalCommandActiveAction",
            "params": {"cortexToken": self.auth, "session": self.session_id, "status": "set", "actions": actions},
        }
    ...
    def get_mental_command_brain_map(self, profile_name):
        ...
        brain_map_request = {
            ...
            "params": {"cortexToken": self.auth, "profile": profile_name, "session": self.session_id},
        }
        if self.debug:
